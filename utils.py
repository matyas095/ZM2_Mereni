...
def round_half_up(value: float, ndigits: int = 0) -> float:
    ...
    Tento helper převede přes `Decimal(repr(float(value)))` (krátká round-trip
    reprezentace), takže `33.05` se interpretuje skutečně jako 33.05, a aplikuje
    `ROUND_HALF_UP`. Cast na `float` je nutný kvůli NumPy 2.x, kde
    `repr(np.float64(x))` vrací `'np.float64(x)'` a `Decimal()` to neumí parsovat.
...
MATH_FUNCTIONS = [
    ...
    # Inverzni trig (Python style)
    'asin', 'acos', 'atan', 'acot', 'asec', 'acsc', 'atan2',
    # Inverzni trig (fyzikalni alias - resi se v sym_map)
    'arcsin', 'arccos', 'arctan', 'arcsinh', 'arccosh', 'arctanh',
    # Hyperbolicke
    'sinh', 'cosh', 'tanh', 'coth', 'sech', 'csch',
    # Inverzni hyperbolicke
    'asinh', 'acosh', 'atanh', 'acoth', 'asech', 'acsch',
    # Ostatni
    'exp', 'log', 'ln', 'sqrt', 'abs', 'Abs', 'floor', 'ceiling', 'Min', 'Max',
    # Konstanty
    'pi', 'e',
]
...
def extract_variables(formula_str, toIgnore=None):
    if toIgnore is None:
        toIgnore = []
    ignored_functions = MATH_FUNCTIONS + toIgnore
...
def smart_parse(rovnice_str):
    ...
    forbidden = {'e', 'pi', 'exp', 'log', 'ln', 'sin', 'cos', 'tan'}
    seen = set()
    clean_vars = []
    for v in found_vars:
