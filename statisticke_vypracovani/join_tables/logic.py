...
class JoinTables(Method):
    ...
    @staticmethod
    def _apply_unit_conversion(headers: list, rows: list, caption: str,
                               si_normalize: bool = False, convert_units: dict = None,
                               dec_sep: str = ","):
        """Převede jednotky v LaTeX headerech a hodnotách. Vrátí (new_headers, new_rows, warnings)."""
        from objects.units import parse_tex_header, format_tex_header, parse_unit, convert_factor, convert_str_value, caption_contains_unit;
        ...
        for i, h in enumerate(headers):
            ...
            if target_unit is not None and target_unit != unit:
                ...
                if caption_contains_unit(caption, unit):
                    warnings.append(f"{color_print.YELLOW}Varování:{color_print.END} caption obsahuje '{unit}' — nepřevedlo se automaticky, zvaž --caption");

        new_rows = [];
        for row in rows:
            new_row = [convert_str_value(v, factors_per_col[i], dec_sep) if i < len(factors_per_col) else v
    ...
    def run(self, args, do_print: bool = True) -> dict:
        ...
        if mode == "match":
            if head1[0] != head2[0]:
                raise Exception(f"Pro --mode match musí být 1. sloupec stejný: IN1='{head1[0]}' vs IN2='{head2[0]}'");
            ...
            for row in rows1:
                ...
                if key in in2_lookup:
                    ...
                    extra = ["-"] * n_extra;
                while len(extra) < n_extra: extra = extra + ["-"];
            ...
            for row in zip_longest(rows1, rows2, fillvalue=None):
                ...
                while len(r1) < len(head1): r1 = r1 + ["-"];
                while len(r2) < len(head2): r2 = r2 + ["-"];
