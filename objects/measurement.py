...
class Measurement:
    ...
    def print_result(self, show_type_b: bool = False, quiet: bool = False) -> None:
        ...
        if self.removed_values:
            ...
            print(f"├──{color_print.YELLOW}Odstraněno outlierů{color_print.END} = {len(self.removed_values)}/{self.original_n} ({', '.join(rem_short)})");
        if show_type_b or self.u_B > 0:
            ...
            print(f"└──{color_print.UNDERLINE}Rozšířená nejistota (k=2){color_print.END} = {self._fmt(self.expanded_uncertainty())}");
        else:
            print(f"├──{color_print.UNDERLINE}Aritmetický průměr{color_print.END} = {self._fmt(self.mean)}");
