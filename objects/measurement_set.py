...
class MeasurementSet:
    ...
    def to_raw_dict(self) -> dict:
        return {
            m.name: list(m.values)
            for m in self.measurements
        };

    def to_numpy(self) -> np.ndarray:
        return np.array(
            [[m.name, list(m.values)] for m in self.measurements],
            dtype=object
        );
    ...
    def to_latex_table(self, source_file: str = None, custom_caption: str = None, custom_label: str = None, dry_run: bool = False):
        import math;
        import os;
        import re;
