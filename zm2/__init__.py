...
def aritmeticky_prumer(
    ...
    args = _make_args(
        input=source, latextable=None, typ_b=typ_b, outliers=outliers,
        caption=None, label=None, verbose=False, dry_run=False,
        export_csv=None, batch=None, si_normalize=si_normalize,
...
def graf(
    ...
    m = Graf();
    args = _make_args(
        input=source, name=name, rovnice=rovnice, parametr=parametr,
        logaritmicky=logaritmicky, fit=fit, chi2=chi2, plot_outliers=plot_outliers,
...
def format_table(
    ...
    args = _make_args(
        input=source, output=output, si_normalize=si_normalize,
        convert_units=conv_str, caption=caption, label=label,
        precision=precision, decimal_separator=",", rows_per_subtable=25,
        append_stats=append_stats, no_caption_stats=False,
        auto_scale=auto_scale, dry_run=False,
    );
    return m.run(args, do_print=False);
...
def join_tables(
    ...
    conv_str = json.dumps(convert_units) if convert_units else None;
    args = _make_args(
        input=[in1, in2], output=output, mode=mode,
        si_normalize=si_normalize, convert_units=conv_str,
    );
    return m.run(args, do_print=False);
