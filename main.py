...
def check_for_updates():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            url = data.get("url")
            if not remote_tag:
                return print("Could not find tag_name in API response.")
...
class CLIApp:
        ...
        for m_name in method_dirs:
            ...
            try:
                ...
                for attr_name in dir(module):
                    ...
                    if inspect.isclass(attr) and issubclass(attr, Method) and attr is not Method:
                        instance = attr()
    ...
    def _build_parser(self):
        ...
        parser.add_argument(
            "--list-units", action="store_true", help="Vypíše podporované SI jednotky a prefixy"
        )
        ALIASES = {
        ...
        for m_name, method_instance in self.methods.items():
            ...
            sub_parser.add_argument("--no-color", action="store_true", help="Vypne barevný výstup")
            sub_parser.add_argument(
                "-q", "--quiet", action="store_true", help="Minimální výstup (jen hodnoty, bez dekorací)"
            )
            try:
