...
class Graf(Method):
    ...
    def _plot_with_residuals(self, x, y, y_pred, sigma, popt, pcov, fit_func, fit_name, args, pathToDir, x_key, y_key):
        ...
        ax_res.errorbar(x, norm_residuals, yerr=1 if sigma is not None and np.all(sigma > 0) else None,
                        fmt='o', capsize=3, color='darkred');
    ...
    def _prep_Plot(self, data, args, pathToDir, ignoreRovnice=False):
        ...
        if args.fit:
            ...
            popt, pcov = curve_fit(fit_func, x_Range, y_Range, absolute_sigma=True,
            ...
            if getattr(args, 'chi2', False):
                ...
                self._plot_with_residuals(x_Range, y_Range, y_pred, sigma, popt, pcov, fit_func, args.fit, args, pathToDir, x_key, y_key);
            else:
                ...
                if sigma is not None:
                    plt.errorbar(x_Range, y_Range, yerr=sigma, fmt='o', capsize=3, color='darkred', label='Chyba měření');
                else:
                    plt.plot(x_Range, y_Range, 'ro', label='Data');
                plt.xlabel(f'{x_key}');
                plt.ylabel(f'{y_key}');
        ...
        if plot_outliers:
            ...
            if outlier_mask.any():
                ...
                plt.plot(x_Range[outlier_mask], y_Range[outlier_mask], 'ro', markersize=10, label=f'Outlier ({plot_outliers})');
